...
#' \dontrun{
#' pl <- list(
#'   Fields = paste(v_fields("Volleyball Statistic"), collapse = " "),
#'   Include = "Teams", SumBy = "Tournament"
#' )
#' cl <- list(
#'   Filter = c(NoTournaments = s_tournament$no, MatchesToUse = "MatchesStarted"),
#'   Relation = c(Name = "Match", Fields = "NoInTournament DateLocal TeamACode TeamBCode MatchResult"),
#'   Relation = c(Name = "Team", Fields = "No Code"),
#'   Relation = c(Name = "Player", Fields = "No TeamName VolleyPosition")
#' )
#' v_get_volley_statistic_list(parent = pl, children = cl)
#' }
#'
#' @export
v_get_volley_statistic_list <- function(parent = list(Fields = paste0(v_fields("Volleyball Statistic"), collapse = " "), Include = "Players", SumBy = "Tournament"), children) {
